...
export function VersionDrawer({ onClose }: VersionDrawerProps) {
  ...
  const { theme, menuPosition, subNavStyle, autoHide, setTheme, setMenuPosition, setSubNavStyle, setAutoHide } = usePreferencesStore();
  ...
  const currentGroupIndex = groupList.findIndex(
    (item) => item.group.id === currentGroup?.id && item.prototypeId === currentPrototype?.id,
  );
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex w-[min(100vw-2rem,22rem)] max-h-[min(80vh,34rem)] flex-col rounded-2xl border border-glass-border bg-glass shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-2xl animate-slide-up overflow-hidden">
        ...
        >
          <div className="min-h-0 overflow-hidden">
            ...
            <div ref={contentRef} className="overflow-y-auto px-3 pb-3" style={{ maxHeight: 'min(55vh, 22rem)' }}>
              {(() => {
                let roundCounter = 0;
                return [...entries.entries()].map(([cat, scenarioList]) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className="flex items-center gap-1.5 flex-1 text-[13px] font-semibold text-foreground">
                        {cat}
                        <span className="rounded-full bg-zinc-700 px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-zinc-400">
                          {scenarioList.length}
                        </span>
                      </span>
                      <ChevronDown
                      ...
                      <div className="min-h-0 overflow-hidden">
                        <div className="space-y-0.5 px-1.5 pb-2">
                          {scenarioList.map((entry) => {
                            const roundNum = `${currentGroupIndex + 1}.${++roundCounter}`;
                            ...
                            return (
                              ...
                              >
                                <span className="truncate text-[13px] font-medium flex items-center gap-1.5">
                                  <span className={cn(
                                    'shrink-0 rounded px-1.5 py-0.5 font-mono text-[10px] font-semibold',
                                    isActive
                                      ? 'bg-primary text-white'
                                      : 'bg-zinc-700 text-zinc-400',
                                  )}>{roundNum}</span>{entry.name}
                                </span>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className={cn(
                          ...
                          <span className={cn(
                            'mr-2 inline-block rounded px-1.5 py-0.5 text-[13px] font-semibold',
                            isActive ? 'bg-primary text-white' : 'bg-zinc-700 text-zinc-400',
                          )}>{chapterNum}</span>{item.group.name}
                        </p>
        ...
        >
          <div className="min-h-0 overflow-y-auto" style={{ maxHeight: 'min(70vh, 30rem)' }}>
            ...
            <div className="space-y-5 px-4 pb-4">
              ...
              {/* Auto Hide */}
              <div>
                <p className="mb-2 text-[12px] font-medium text-muted-foreground">Auto Hide</p>
                <div className="flex gap-2">
                  {([
                    { value: true, icon: EyeOff, label: 'On' },
                    { value: false, icon: Eye, label: 'Off' },
                  ] as const).map((opt) => (
                    <button
                      key={String(opt.value)}
                      type="button"
                      onClick={() => setAutoHide(opt.value)}
                      className={cn(
                        'flex flex-1 items-center justify-center gap-2 rounded-xl border py-2.5 text-[13px] font-semibold transition-all duration-150',
                        autoHide === opt.value
                          ? 'border-primary/40 bg-primary/10 text-foreground'
              ...
              {focusSelector && (
                <div>
                  <p className="mb-2 text-[12px] font-medium text-muted-foreground">Focus Area</p>
                  <button
                    type="button"
                    onClick={() => clearFocusForRound()}
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-destructive/30 bg-destructive/5 py-2.5 text-[13px] font-semibold text-destructive transition-all duration-150 hover:bg-destructive/10"
                  >
                    Clear focus area
                  </button>
                </div>
